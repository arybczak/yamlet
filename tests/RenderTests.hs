  ...
  where
    ...
    doc = Document
      ...
      , root = mappingNode
          ...
          , (withEmptyLine (plainNode "jobs"), mappingNode
              [ (withEmptyLine (plainNode "build"), mappingNode
...
test_attachment = do
  ...
  assertEqual "empty line" (Right [EmptyLine])
...
test_movedComments = do
  ...
  assertEqual "lines above a value on the line of the key" "# v\na: 1\n"
    (render (mappingNode [(plainNode "a", withBefore "v" (plainNode "1"))]))
  assertEqual "two comments on one line" "# k\na: 1 # v\n"
    (render (mappingNode [(withInline "k" (plainNode "a"), withInline "v" (plainNode "1"))]))
  assertEqual "comment in a flow sequence" "a:\n- 1 # c\n- 2\n"
...
prop_roundTrip (Tree doc) =
  ...
  where
    ...
    item inFlow n = case (n.props, n.content) of
      (Props Nothing NoTag, Scalar Plain "") | inFlow ->
    ...
    hasComments n = not (null [ () | Comment _ <- n.comments.after ]) || case n.content of
      Sequence _ xs -> any inner xs
      Mapping _ kvs -> any (\(k, v) -> inner k || inner v) kvs
      _ -> False
      where
        inner :: Node -> Bool
        inner x = not (null [ () | Comment _ <- x.comments.before ])
          || x.comments.inline /= Nothing || hasComments x
...
genNode size = do
  n <- if size <= 1 then genScalar else frequency
    [ (3, genScalar)
...
genComments collection = frequency
  [ (3, pure noComments)
  , (1, Comments <$> genLines <*> oneof [pure Nothing, Just <$> genCommentText]
...
firstDifference a b = go (1 :: Int) "" (T.lines a) (T.lines b)
  where
    ...
    go n prev xs ys = case (xs, ys) of
      ...
      _ -> "line " ++ show n ++ " after " ++ show prev ++ ": "
        ++ show (take 1 xs) ++ " /= " ++ show (take 1 ys)
